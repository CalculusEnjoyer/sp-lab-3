...
void print_highlighted_text(const std::string &text, LexemeTable &table) {
    ...
    for (auto item : table_v) {
        ...
        for (int i = next_pos; i < item.position ; i++) {
            if (std::isspace(text[i]) || text[i] == '\n') {
                set_background_color(BackgroundColor::RESET);
            } else {
                set_background_color(BackgroundColor::UNDEF);
            }
            std::cout << text[i];
        }
    ...
    set_background_color(BackgroundColor::RESET);
    for (int i = next_pos; i < text.length(); i++) {
        if (std::isspace(text[i]) || text[i] == '\n') {
            // Whitespace or newline, print in the default color
            set_background_color(BackgroundColor::RESET);
        } else {
            // Unrecognized character, print in red
            set_background_color(BackgroundColor::UNDEF);
        }
        std::cout << text[i];
    }
